...
class TreeBuilder:
    ...
    def create_node(
        ...
        return (index, Node(text, index, children_indices, embeddings, metadata))
    ...
    def build_from_documents(
        ...
        """
        Builds a RAPTOR tree from pre-chunked documents.

        Each input document becomes exactly one leaf node, preserving metadata
        needed for retrieval evaluation.
        """
        if not documents:
            raise ValueError("documents must contain at least one item")

        def create_document_node(index, document):
            text = str(document.get(text_key, "")).strip()
            if not text:
                raise ValueError(f"document at index {index} has empty text")
            metadata = dict(document.get(metadata_key, {}) or {})
            metadata.setdefault("source_index", index)
            metadata.setdefault("node_type", "leaf")
            return self.create_node(index, text, metadata=metadata)

        logging.info("Creating Leaf Nodes From Documents")

        if use_multithreading:
            with ThreadPoolExecutor() as executor:
                future_nodes = {
                    executor.submit(create_document_node, index, document): index
            ...
            leaf_nodes = {}
            for index, document in enumerate(documents):
                __, node = create_document_node(index, document)
                leaf_nodes[index] = node
