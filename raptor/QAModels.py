...
class GPT3QAModel(BaseQAModel):
    def __init__(self, model="text-davinci-003"):
        ...
        """
        if OpenAI is None:
            raise ImportError("openai is required for GPT3QAModel.")
        self.model = model
...
class GPT3TurboQAModel(BaseQAModel):
    def __init__(self, model="gpt-3.5-turbo"):
        ...
        """
        if OpenAI is None:
            raise ImportError("openai is required for GPT3TurboQAModel.")
        self.model = model
        self.client = OpenAI(api_key=os.environ["OPENAI_API_KEY"])
...
class GPT4QAModel(BaseQAModel):
    def __init__(self, model="gpt-4"):
        ...
        """
        if OpenAI is None:
            raise ImportError("openai is required for GPT4QAModel.")
        self.model = model
        self.client = OpenAI(api_key=os.environ["OPENAI_API_KEY"])
...
class UnifiedQAModel(BaseQAModel):
    def __init__(self, model_name="allenai/unifiedqa-v2-t5-3b-1363200"):
        if torch is None or T5ForConditionalGeneration is None or T5Tokenizer is None:
            raise ImportError("torch and transformers are required for UnifiedQAModel.")
